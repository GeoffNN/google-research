...
class ImageGraph(graph_api.GraphAPI):
  ...
  def task_features(self):
    ...
    """
    return self.node_features(self._start_node_id)
...
@flax.struct.dataclass
class PatchImageGraph(graph_api.GraphAPI):
  """Graph representation of an image, by splitting the image in non-overlapping patches.
  
  In this representation, each node in the graph is a patch. 
  Each patch is connected to its 8 neighbors, or to a "border" node.
  """
  ...
  @staticmethod
  def _pixel_coords_to_patch_coords(pixel_coords: jnp.array, patch_size: int):
    """Converts pixel coordinates to patch coordinates.
  ...
  def _patch_coordinates_to_id(self, node_hw):
    node_id = self.patch_image_shape[1] * node_hw[0] + node_hw[1]
    return jnp.where(node_id < 0, -1, node_id)

  def _patch_id_to_coordinates(self, node_id):
    """Converts pixel id to coordinates.
    ...
    # Can't use jnp.unravel_index due to the -1 node.
    coordinates = jnp.array(
        (node_id // self.patch_image_shape[1], node_id % self.patch_image_shape[1]))
    coordinates = jnp.where(
        jnp.all(coordinates >= 0, axis=-1, keepdims=True), coordinates, -1)
    return coordinates
  ...
  def create(image,
    ...
    Args:
      image: The image to be represented as a graph.
      get_start_pixel_fn: This function produces the location of the start
        patch as a pair of (h, w) coordinates.
      patch_size: Size of patches to use as nodes.
      num_colors: Number of colors in the image.
      padding_value: Value to insert on the boundary of the pixel for computing
        patch features of pixels on the boundary.
    ...
    return PatchImageGraph(
        ...
        image_shape=image.shape,
        patch_size=patch_size,
        patch_image_shape=patch_image_shape,
        padding_value=padding_value,
        window_size=window_size,
        _start_node_id=start_node_id,
        _num_colors=num_colors)
  ...
  def node_patch(self, node_id):
    """Returns the features for a given node.
    
    The node is represented by a patch of the image.
    """
    patch_coordinates = self._patch_id_to_coordinates(node_id)
    center_pixel_coordinates = patch_coordinates * self.patch_size + self.window_size
  ...
  def _outgoing_edges_in_bounds_pixel(self, node_id, relation_ids):
    ...
    This "out of bounds" node points to the start node.
    The considered initial node must be inside the image bounds.
    ...
    Returns:
      A list of neighbor ids, in the order given by relation_ids.
    """
    node_coordinates = self._patch_id_to_coordinates(node_id)
    neighbor_coordinates = jnp.repeat(
        jnp.array(node_coordinates)[jnp.newaxis, :], len(self.RELATION_OFFSETS),
        0)
    offsets = jnp.array(self.RELATION_OFFSETS)
    ...
    neighbor_coordinates = jnp.where(
        jnp.less_equal(neighbor_coordinates,
                       jnp.array(self.patch_image_shape) - 1), neighbor_coordinates,
        -1)
    # make sure coordinates are >= 0, or -1
    neighbor_coordinates = jnp.where(
        jnp.all(neighbor_coordinates >= 0, axis=-1, keepdims=True),
